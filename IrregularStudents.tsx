...
const IrregularStudentsPage: React.FC = () => {
  ...
  const loadIrregularStudents = async () => {
    setIsLoading(true);
    try {
      const mockStudents: IrregularStudent[] = [
        { id: 1, name: "Ahmed Mohamed", university_id: "443344567", level: 8, remaining_courses: ["SWE211", "CSC113", "MATH244"], created_at: "2024-01-15" },
        { id: 2, name: "Sara Ali", university_id: "444299111", level: 7, remaining_courses: ["CSC220", "CSC212"], created_at: "2024-01-20" }
      ];
      setStudents(mockStudents);
    } catch (error) {
      setAlert({type: 'danger', message: 'Failed to load irregular students'});
    } finally {
      setIsLoading(false);
    }
  };

  const createIrregularStudent = async (payload: Omit<IrregularStudent, 'id'>) => {
    const newStudent: IrregularStudent = { id: Date.now(), ...payload };
    return newStudent;
  };

  const deleteIrregularStudent = async (id: number) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const payload: Omit<IrregularStudent, 'id'> = {
