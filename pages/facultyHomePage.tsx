...
const FacultyHomePage: React.FC = () => {
  ...
  const fetchSchedule = async () => {
    ...
    setAlert(null);
    try {
      // Fetch schedule for the faculty member by instructor name
      const response = await fetch(`/api/data/schedule?instructor=${encodeURIComponent(faculty.instructor_name)}`);
      const data = await response.json();
      if (data.success && data.entries) {
        ...
        setAlert({
          type: 'warning',
          message: `No schedule entries found for ${faculty.instructor_name}`
        });
      }
    } catch (error) {
        ...
        >
          <div className="d-flex justify-content-between align-items-center">
            <h5 className="mb-0 fw-semibold">
              ...
              My Teaching Schedule - {faculty?.instructor_name}
            </h5>
