...
async function handlePost(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  ...
  try {
    ...
    const prerequisiteCheck = await client.query(
      `SELECT cp.prerequisite_course_code, c.course_name, c.level AS prereq_level
    ...
    if (prerequisiteCheck.rows.length > 0) {
      ...
      for (const prereq of prerequisiteCheck.rows) {
        // If the prerequisite course is from a lower level than the target level,
        // we assume students have already taken it in prior levels and do not
        // require it to be present in the current level's schedule.
        if (typeof prereq.prereq_level === 'number' && prereq.prereq_level < targetLevel) {
          continue;
        }

        // For same-level (co-requisite) or higher-level prerequisites, ensure it's scheduled
